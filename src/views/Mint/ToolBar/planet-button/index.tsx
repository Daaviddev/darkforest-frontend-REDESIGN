...
function PlanetButton({ action, planetId, actionTime }: IPlanetButtonProps) {
    ...
    if (action == "create") {
        ...
        buttonText = timeLeft == 0 ? "Compound All" : new Date(timeLeft * 1000).toISOString().substring(11, 19);
        className = timeLeft == 0 ? "planet-button" : "planet-button disabled";
        clickFunc = () => {
            onCompoundAll();
        };
    } else if (action == "claimall") {
        buttonText = timeLeft == 0 ? "Claim All" : new Date(timeLeft * 1000).toISOString().substring(11, 19);
        className = timeLeft == 0 ? "planet-button" : "planet-button disabled";
        clickFunc = () => {
            onClaimAll();
        };
